(function(global, factory) {
  ...
  function transformer() {
    ...
    scale.domain = function(_) {
      return arguments.length
        ? ((domain = map$2.call(_, number$1)),
          clamp === identity$3 || (clamp = clamper(domain)),
          rescale())
        : domain.slice();
    };
    ...
    scale.range = function(_) {
      return arguments.length
        ? ((range = slice$1.call(_)), rescale())
        : range.slice();
    };

    scale.rangeRound = function(_) {
      return (
        (range = slice$1.call(_)), (interpolate = interpolateRound), rescale()
      );
    };

    scale.clamp = function(_) {
      return arguments.length
        ? ((clamp = _ ? clamper(domain) : identity$3), scale)
        : clamp !== identity$3;
    };
  ...
  function arc() {
    ...
    arc.padRadius = function(_) {
      return arguments.length
        ? ((padRadius =
            _ == null ? null : typeof _ === "function" ? _ : constant$3(+_)),
          arc)
        : padRadius;
    };
  ...
  function pie() {
    ...
    function pie(data) {
      var i,
        ...
        da = Math.min(
          tau$1,
          Math.max(-tau$1, endAngle.apply(this, arguments) - a0)
        ),
        a1,
  ...
  const drawBar = barArg => {
    ...
    const xScale = drawLinearAxis({
      type: barArg.type,
      domainMin: 0,
      domainMax: max(barArg.data, d => d.xAxisLabel),
      rangeMin: 0,
  ...
  const drawLine = lineArg => {
    ...
    const yScale = drawLinearAxis({
      type: lineArg.type,
      domainMin: 0,
      domainMax: max(lineArg.data, d => d.yAxisLabel),
  ...
  const drawPie = pieArg => {
    const pie$1 = pie()(pieArg.pieData.map(d => d.yAxisLabel));
  ...
  const drawColumn = columnArg => {
    ...
    const yScale = drawLinearAxis({
      type: columnArg.type,
      domainMin: 0,
      domainMax: max(columnArg.data, d => d.yAxisLabel),
  ...
  const draw = data => {
    ...
    const dataSorted = data.value.sort((a, b) =>
      ascending(a.xAxisLabel, b.xAxisLabel)
    );
